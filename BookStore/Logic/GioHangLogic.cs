...
namespace BookStore.Logic
{
	public class GioHangLogic
	{
		...
		public GioHangLogic(BookStoreDBContext context)
		{
			_context = context;
		} 
		...
		public decimal LayTongTienSanPham()
		{
			...
			tongTien = (decimal?)(from r in _context.GioHang
								  ...
								  select (decimal?)r.SoLuongTrongGio * r.Sach.DonGia).Sum();
		...
		{
			_tenDangNhap = LayTenDangNhap();
			return _context.GioHang.Where(r => r.TenDangNhap == _tenDangNhap).Include(s => s.Sach).ToList();
		}
		public void ThemSanPham(int maSanPham)
		{
			_tenDangNhap = LayTenDangNhap();
			var gioHang = _context.GioHang.SingleOrDefault(r => r.TenDangNhap == _tenDangNhap && r.SachID == maSanPham);
			if (gioHang == null)
			{
				gioHang = new GioHang
			{
					...
					SachID = maSanPham,
					Sach = _context.Sach.SingleOrDefault(r => r.ID == maSanPham),
					SoLuongTrongGio = 1,
